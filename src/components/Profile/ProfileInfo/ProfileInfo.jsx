...
const ProfileInfo = (props) => {
  ...
  let coverPhoto = oftenCheckMyProfile && props.account.profile.coverPhoto ? props.account.profile.coverPhoto : oftenCheckOtherProfile && otherProfile.profile.coverPhoto ? otherProfile.profile.coverPhoto : undefined;
  let checkFollow = props.account && props.account.profile && props.account.profile.following ? props.account.profile.following.find((account) => account.id === props.id) : undefined;
    ...
    <div className={styles.profile_info}>
      <div className={styles.profile_cover} style={{ backgroundImage: `linear-gradient(rgba(0, 0, 0, 0.3), rgba(0, 0, 0, 0.3)), url(${coverPhoto})` }}>
        <div className={styles.profile_cover_line}>
          {/* full name */}
          <div className={styles.profile_fullName}>{oftenCheckOtherProfile ? otherProfile.profile.surname + " " + otherProfile.profile.name : oftenCheckMyProfile ? props.account.profile.surname + " " + props.account.profile.name : <></>}</div>

          {/* status */}
          <div className={styles.profile_status}>{oftenCheckOtherProfile && otherProfile.profile.status ? <div>{otherProfile.profile.status}</div> : oftenCheckMyProfile && props.account.profile.status ? <div>{props.account.profile.status}</div> : undefined}</div>
        </div>

        {/* wrapper picture */}
        <div className={openModalAvatarProfile && !props.id ? styles.wrapper_profilePicture_active__center : styles.wrapper_profilePicture__center}>{oftenCheckMyProfile && props.account.profile.avatar ? <img onClick={() => (openModalAvatarProfile ? setOpenModalAvatarProfile(false) : setOpenModalAvatarProfile(true))} src={props.account.profile.avatar} title="Change profile photo" alt="" /> : oftenCheckOtherProfile && otherProfile.profile.avatar ? <img src={otherProfile.profile.avatar} alt="" /> : !props.id ? <img onClick={() => (openModalAvatarProfile ? setOpenModalAvatarProfile(false) : setOpenModalAvatarProfile(true))} src={defaultAvatar} title="Change profile photo" alt="" /> : <img src={defaultAvatar} alt="" />}</div>
      ...
      <div className={styles.profile_info_line}>
        ...
        <div className={styles.wrapper_profile_details_info_line}>
          {/* full name */}
          <div className={styles.profile_fullName__media}>{oftenCheckOtherProfile ? otherProfile.profile.surname + " " + otherProfile.profile.name : oftenCheckMyProfile ? props.account.profile.surname + " " + props.account.profile.name : <></>}</div>

          {/* status */}
          <div className={styles.profile_status__media}>{oftenCheckOtherProfile && otherProfile.profile.status ? <div>{otherProfile.profile.status}</div> : oftenCheckMyProfile && props.account.profile.status ? <div>{props.account.profile.status}</div> : undefined}</div>

          <div className={styles.details_info_content}>
            <div className={styles.details_info}>
              <div className={styles.detail_number}>{oftenCheckOtherProfile && otherProfile.profile.posts && otherProfile.profile.posts.length > 0 ? otherProfile.profile.posts.length : 0 || (oftenCheckMyProfile && props.account.profile.posts && props.account.profile.posts.length > 0) ? props.account.profile.posts.length : 0}</div>
              <div className={styles.detail_title}>Posts</div>
            </div>
            <div className={styles.details_info}>
              <div className={styles.detail_number}>{oftenCheckOtherProfile && otherProfile.profile.followers && otherProfile.profile.followers.length > 0 ? otherProfile.profile.followers.length : 0 || (oftenCheckMyProfile && props.account.profile.followers && props.account.profile.followers.length > 0) ? props.account.profile.followers.length : 0}</div>
