...
const Comments = (props) => {
  ...
  let replaceToPost = () => {
    // window.location.href = `${photoConstant}/${props.post.id}`;
    // <Redirect to={`${photoConstant}/${props.post.id}`} />;
  };
  ...
  return (
    <div className={!props.modal ? styles.comment_content : styles.comment_content_modal}>
      ...
      {/* comments */}
      {props.post && props.post.comments && props.modal ? props.post.comments.map((comment) => <Comment {...props} key={comment.comment} post={props.post} comment={comment} />) : undefined}

      {props.post && props.post.comments && props.post.comments.length > 0 && !props.modal ? <div onClick={() => props.history.push(`${photoConstant}/${props.post.id}`)} className={styles.button_more_comments}>{`Переглянути всі коментарі: ${props.post.comments.length}`}</div> : undefined}
    </div>
  );
};
