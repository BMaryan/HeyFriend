...
const Edit = (props) => {
  ...
  return (
    <div className={styles.edit}>
      {/* head for go back */}
      {checkProfile || checkPassword || checkManageAccess ? <Media query={{ maxWidth: 399 }}>{(matches) => (matches ? <GoBackHead title={checkProfile ? title.profile : checkPassword ? title.password : checkManageAccess ? title.appsAndWeb : undefined} /> : <></>)}</Media> : undefined}
